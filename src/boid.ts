...
export class Boid extends THREE.Mesh {
  ...
  constructor(
    geometry: THREE.CylinderGeometry = new THREE.CylinderGeometry(1, 8, 25, 12),
    ...
    this.velocity.set(
      THREE.MathUtils.randFloat(-100, 100) * 0.1,
      THREE.MathUtils.randFloat(-100, 100) * 0.1,
      THREE.MathUtils.randFloat(-100, 100) * 0.1
    );
  }
  ...
  public calculateSteering(
    boids: Boid[],
    getVector: (boid: Boid) => THREE.Vector3,
    config: {
      force: string;
      maxSpeed: number;
      maxForce: number;
  ...
  public align(boids: Boid[], settings: BoidSettings): THREE.Vector3 {
    return this.calculateSteering(boids, (boid: Boid) => boid.velocity, {
      force: "align",
      maxSpeed: settings.maxSpeed,
      maxForce: settings.maxForce,
      VecDistance: settings.alignmentDist,
    });
  }
  ...
  public separate(boids: Boid[], settings: BoidSettings): THREE.Vector3 {
    return this.calculateSteering(
      ...
      },
      {
        force: "separate",
        maxSpeed: settings.maxSpeed,
        maxForce: settings.separationWeight,
        VecDistance: settings.separationDist,
      }
    );
  }
  ...
  public avoidBoxContainer(rangeWidth = 80, rangeHeight = 80, rangeDepth = 80) {
    ...
    sumVector.add(
      this.avoid(
        new THREE.Vector3(rangeWidth, this.position.y, this.position.z)
      )
    );
    sumVector.add(
      this.avoid(
    ...
    sumVector.add(
      this.avoid(
        new THREE.Vector3(this.position.x, rangeHeight, this.position.z)
      )
    );
    sumVector.add(
      this.avoid(
    ...
    sumVector.add(
      this.avoid(
        new THREE.Vector3(this.position.x, this.position.y, rangeDepth)
      )
    );
    sumVector.add(
      this.avoid(
  ...
  public avoidSphereContainer(radius = 100) {
    ...
    if (!boundingSphere) return new THREE.Vector3();
    const distance =
      radius - this.position.length() - boundingSphere.radius * 2;
...
export class Boids {
  ...
  public getRandomBoid() {
    return this.boidsGroup.children[
      Math.floor(Math.random() * this.boidsGroup.children.length)
    ] as Boid;
  }
}
...
export class BoidCamera extends THREE.PerspectiveCamera {
  ...
  public updateToCenter(center: THREE.Vector3) {
